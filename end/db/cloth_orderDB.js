...
module.exports = {
    ...
    save(order){
      ...
      var sql = "insert into cloth_order values(null,"+order.number+","+order.price+","+order.user_id+","+order.cloth_id+","+order.address+",'"+order.date+"','"+order.color+"','"+order.style+"','"+order.details+"','未付款')";
      console.log(sql);
      return pool.execute(sql);
    },
    //更新
    ...
    fukuan(id,address_id){
      var sql = "update cloth_order set status ='已付款',address_id="+address_id+" where id ="+id;
      console.log(sql);
      return pool.execute(sql);
    },
    fahuo(id){
      var sql = "update cloth_order set status ='已发货' where id ="+id;
      return pool.execute(sql);
    }
}
