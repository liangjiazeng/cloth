...
//付款
route.post('/fukuan',(req,resp)=>{
  var id = JSON.parse(req.body.id);
  var address_id = JSON.parse(req.body.address_id);
  console.log("付款的id",id);
  console.log("address_id",address_id);
  cloth_orderDB.fukuan(id,address_id).then((data)=>{
    resp.send(data);
  }).catch((error)=>{
    resp.send(error);
  });
...
//发货
route.post('/fahuo',(req,resp)=>{
  var id = JSON.parse(req.body.id);
  console.log("发货的id",id);
  cloth_orderDB.fahuo(id).then((data)=>{
    resp.send(data);
  }).catch((error)=>{
    resp.send(error);
  });
