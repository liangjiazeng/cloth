        ...
        <el-form :model="ruleForm2" status-icon  ref="ruleForm2" label-width="0px" class="demo-ruleForm">
          ...
          <el-form-item label="" prop="checkPass" required id="regesiter_repassword">
           ...
           <el-form-item label="" prop="checkPass" required id="regesiter_tel">
            ...
            <div class="right">
              <el-input type="text" placeholder="请输入手机号" v-model="ruleForm2.telphone" auto-complete="off" pattern="(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$" style="width:250px"></el-input>
            </div>
          </el-form-item> 
          <template>
...
methods:{
    ...
    user_login:function() {
          var name = this.ruleForm1.username;
            ...
            if(name=='admin'&&password=='admin'){
              ...
              this.userlogin(this.ruleForm1).then(function(data){
                if(data.data[0]!=undefined){
                    ...
                    vm.Title ="用户名或密码不正确，请重新登录!"
                    vm.dialogVisible= true;
                    vm.ruleForm1.username = "";
                    vm.ruleForm1.password = "";
                 }               
      ...
      resetForm(){
        ...
        this.ruleForm2.rule ="普通用户";
        this.ruleForm2.pass ="";
        this.ruleForm2.checkPass ="";
        this.ruleForm2.gender ="男";
        this.ruleForm2.birth ="";
        this.ruleForm2.telphone="";
      },
      user_regester:function(){
          ...
          var vm = this;
          if(!(/^1[3|4|5|8][0-9]\d{8}$/.test(telphone))){
              vm.Title ="请重新输入11位正确手机号!"
              vm.dialogVisible= true;
              this.resetForm();
          }else{
            if(password!=repass){
              ...
              if(user_rule == "普通用户"){
                axios.post('http://39.107.228.71:3000/user/findByName',{name:JSON.stringify(name)}).then((data)=>{
                  if(data.data.length>0){
                    vm.Title ="该用户名已经注册过,请重新注册!"
                    vm.dialogVisible= true;
                    this.resetForm();
                  ...
                  // 如果是设计师就注册到设计师的表中
                    ...
                    axios.post('http://39.107.228.71:3000/designer/findByName',{designerName:JSON.stringify(name)}).then((data)=>{
                      if(data.data.length>0){
                        vm.Title ="该用户名已经注册过,请重新注册!"
                        vm.dialogVisible= true;
                        this.resetForm();
