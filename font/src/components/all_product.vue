ss<template>
  ...
  <div class="pinpai">
      <ul>
        <li>设计师：</li>
        <li v-for="item in designers"><a @click="findClothesByDesigner(item.id)">{{item.name}}</a></li>
      </ul>
  </div>
          ...
          <div class="cart-grid" id="cart-1">
            ...
            <div class="snipcart-details ">
              <form action="#" method="post">
                ...
                <div type="" class="button w3l-cart" data-id="cart-1" style="margin-top:10px;" @click="to_shoucang(item.id,userName)">收藏</div>
              </form>
            </div>
          </div>
...
export default {
  ...
  created(){
    ...
    this.designer_id = this.$route.params.designer_id;
    if( this.designer_id){
      console.log("this.designer_id ",this.designer_id );
      this.findClothesByDesigner(this.designer_id);
    }else{
      this.FindAllClothes();
    }
  },
  ...
  methods:{
    ...
    to_shoucang(cloth_id,username){
      if(username){
        axios.post('http://39.107.228.71:3000/user/saveLikeCloth',{cloth_id:JSON.stringify(cloth_id),username:username}).then((result)=>{
           this.$message({
              showClose: true,
              type: 'success',
              message: '收藏成功!'
